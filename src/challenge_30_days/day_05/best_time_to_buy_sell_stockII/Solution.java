...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        while (current < prices.length)
            if (current < prices.length -1 && prices[current] > prices[current + 1])
            {
                current++;
                continue;
            }
                ...
                int localMin = prices[current];
                current++;
                while (current < prices.length - 1 && prices[current] < prices[current + 1]) {
                    current++; // to find next local maximum
                }
                if (current < prices.length) {

                    max = max + prices[current] - localMin;
                }
                current++;
            }
        return max;
